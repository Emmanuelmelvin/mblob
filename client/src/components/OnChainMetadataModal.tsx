...
export function OnChainMetadataModal({ metadata, onClose }: { metadata: OnChainBlobMetadata; onClose: () => void }) {
    const rows = [
        ...
        ['Expires at', metadata.expiresAt === '0' ? 'Permanent / no expiry' : metadata.expiresAt],
        ['Replication factor', String(metadata.replicationFactor)],
        ['Deletable', metadata.deletable ? 'Yes' : 'No'],
        ['Payment wei', metadata.payment],
    ] as const

    return <div className="fixed inset-0 z-50 bg-black/50 px-4 py-8 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="On-chain blob metadata">
