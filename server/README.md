...
- `POST /v1/blobs/:blobId/upload` — signed raw request body containing the plaintext file bytes. The browser sends `content-type`, `x-file-hash`, optional `x-file-name`, and the signature headers.
