...
- `POST /v1/blobs/:blobId/upload` — signed multipart form-data request with a `file` field. Legacy raw file-body uploads are also accepted with `content-type` and optional `x-file-name` headers.
